...
-su
: 'allow su' allow process to escalate privilege (e.g. su to root user)
-pids <n>
: max child pids (requires linux namespaces to work)
-upids <n>
: max system-wide pids owned by the user of the run program
-files <n>
: max files open by run program
-mem <size>
: max memory in bytes. 'size' accepts suffixes like k, M and G
-fsize <size>
: max file size in bytes. 'size' accepts suffixes like k, M and G
-nonet
: unshare network namespace, preventing all network access
-nopid
: unshare pid namespace, preventing access to other processes
-noipc
: unshare ipc namespace, preventing access shared memory, message queues and semaphores
-sec <config>
: set security config, see --help-secure for more
-secure <config>
: set security config, see --help-secure for more
-U <user>
: Run as user
